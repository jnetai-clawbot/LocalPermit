...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = PermitAdapter { item -> startActivity(Intent(this, DetailActivity::class.java).putExtra("item_id", item.id)) }
    ...
    private fun refreshList() {
        lifecycleScope.launch {
            val items = app.database.dao().getAllPermits()
            (binding.recyclerView.adapter as PermitAdapter).items = items
            (binding.recyclerView.adapter as PermitAdapter).notifyDataSetChanged()
